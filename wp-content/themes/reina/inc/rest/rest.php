...
if ( ! class_exists( 'ReinaRestAPI' ) ) {
	...
	class ReinaRestAPI {
		...
		public function __construct() {
			// Init variables
			$this->set_namespace( 'reina/v1' );
			
			// Localize theme's main js script with rest variable
			add_filter( 'reina_filter_localize_main_js', array( $this, 'localize_script' ) );
		...
		function localize_script( $global ) {
			...
			return apply_filters( 'reina_filter_rest_api_global_variables', $global, $this->get_namespace() );
		}
		
		function register_rest_api_route() {
			$routes = apply_filters( 'reina_filter_rest_api_routes', array() );
