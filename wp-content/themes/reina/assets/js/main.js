(function ($) {
	...
	var qodefSwiper = {
		...
		getOptions: function ($holder, returnBreakpoints) {
			var sliderOptions = typeof $holder.data('options') !== 'undefined' ? $holder.data('options') : {},
				...
				grabCursor = sliderOptions.grab !== undefined && sliderOptions.grab !== '' ? sliderOptions.grab : true
	...
	/**
	 * Init svg draw on appear
	 */
	var qodefDrawSvg = {
		init: function () {
			var holder = $('.qodef--draw-svg, #qodef-fullscreen-area-icon, #qodef-side-area-image, .qodef-row-svg-animation-yes .qodef-row-background-svg');

			function getRandomArbitrary(min, max) {
				return Math.floor(Math.random() * (max - min) + min);
			}

			if (holder.length && !holder.parents('.single-format-quote, .single-format-link').length) {
				holder.each(function () {
					var thisHolder = $(this),
						thisItem = thisHolder.find('svg'),
						thisItemPath = thisHolder.find('path'),
						randomNum = getRandomArbitrary(10, 400);

					thisItemPath.each(function(){
						var line = $(this),
							len = Math.round(this.getTotalLength()),
							duration = parseFloat(len) + 'ms';

						if (parseFloat(len)>1500){
							var lenShort = 1500,
								durationShort = '1500ms';

							if (parseFloat(len)>3000){
								lenShort = 3000;
								durationShort = '3500ms';
							}

							line.css({
								'stroke-dasharray': len,
								'stroke-dashoffset': lenShort,
								'animation-duration': durationShort,
							});
						} else {
							line.css({
								'stroke-dasharray': len,
					...
					if (!thisHolder.hasClass('qodef--draw-svg-random-delay')) {
						thisItem.appear(function () {
							thisHolder.addClass('qodef--svg-drawn');
						}, {accX: 0, accY: -50});
					} else {
						thisItem.appear(function () {
							setTimeout(function () {
	...
	qodef.qodefDrawSvg = qodefDrawSvg;
		...
		addHtml: function (button){
			if (button.length) {
				button.each(function () {
					...
					if (!thisButton.find('.qodef-m-background').length) {
						thisButton.wrapInner( "<div class='qodef-m-text'></div>" );
						thisButton.append("<span class='qodef-m-background'></span>");
					}
				});
			}
		}
	 ...
	 */
	var qodefParallaxOnScroll = {
		init: function () {
			var $holder = $('.qodef-parallax-image');
			qodefParallaxOnScroll.initParallax($holder);
		},

		initParallax: function (holder){
			if($("html").hasClass("no-touchevents") && holder.length){

				holder.each(function(){
					var itemHolder = $(this),
						item = itemHolder.find('img'),
						delta = 120,
						smoothness = 20;

					// if ($('body').hasClass('qodef-browser--firefox')){
					// }
...
(function ($) {
    ...
    $(document).on('reina_trigger_get_new_posts', function (e, holder) {
...
(function ($) {
	...
	var qodefFilter = {
		...
		setVisibility: function ($holder, $item) {
			...
			qodef.body.trigger('reina_trigger_load_more', [$holder, 1]);
		},
...
(function ($) {
    ...
    $(document).on('reina_trigger_load_more', function (e, $holder, nextPage) {
                ...
                success: function (response) {
                    ...
                    if (response.status === 'success') {
                        ...
                        $itemsHolder.waitForImages(function () {
                            ...
                            setTimeout(function () {
                                qodef.body.trigger('reina_trigger_get_new_posts', [$holder, response.data, nextPage]);
