...
if ( ! function_exists( 'reina_core_add_page_logo_meta_box' ) ) {
	/**
	 * Function that add general meta box options for this module
	 *
	 * @param object $page
	 */
	function reina_core_add_page_logo_meta_box( $page ) {
		...
		if ( $page ) {
			...
			$logo_tab = $page->add_tab_element(
				array(
					'name'        => 'tab-logo',
					'icon'        => 'fa fa-cog',
					'title'       => esc_html__( 'Logo Settings', 'reina-core' ),
					'description' => esc_html__( 'Logo settings', 'reina-core' )
				)
			);
			...
			do_action( 'reina_core_action_after_page_logo_meta_map', $logo_tab, $header_logo_section );
		}
	}

	add_action( 'reina_core_action_after_general_meta_box_map', 'reina_core_add_page_logo_meta_box' );
}

if ( ! function_exists( 'reina_core_add_general_logo_meta_box_callback' ) ) {
	/**
	 ...
	 * @return array
	 */
	function reina_core_add_general_logo_meta_box_callback( $callbacks ) {
		$callbacks['logo'] = 'reina_core_add_page_logo_meta_box';
		
		return $callbacks;
	}
	
	add_filter( 'reina_core_filter_general_meta_box_callbacks', 'reina_core_add_general_logo_meta_box_callback' );
}