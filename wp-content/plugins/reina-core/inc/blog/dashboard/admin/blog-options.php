...
if ( ! function_exists( 'reina_core_add_blog_options' ) ) {
	/**
	 * Function that add general options for this module
	 */
	function reina_core_add_blog_options() {
		...
		$page = $qode_framework->add_options_page(
			array(
				...
				'slug'        => 'blog',
				'icon'        => 'fa fa-cog',
				'title'       => esc_html__( 'Blog', 'reina-core' ),
				'description' => esc_html__( 'Global Blog Options', 'reina-core' ),
				'layout'      => 'tabbed'
			)
		);
		...
		if ( $page ) {
			...
			$list_tab = $page->add_tab_element(
				array(
					'name'        => 'tab-list',
					'icon'        => 'fa fa-cog',
					'title'       => esc_html__( 'Blog List', 'reina-core' ),
					'description' => esc_html__( 'Settings related to blog list', 'reina-core' )
				)
			);
			...
			$list_tab->add_field_element(
				array(
					...
					'description'   => esc_html__( 'Choose default sidebar layout for blog archive', 'reina-core' ),
			...
			if ( ! empty( $custom_sidebars ) && count( $custom_sidebars ) > 1 ) {
				$list_tab->add_field_element(
					array(
						...
						'description'   => esc_html__( 'Choose a custom sidebar to display on blog archive', 'reina-core' ),
			...
			$list_tab->add_field_element(
				array(
					...
					'description'   => esc_html__( 'Choose grid gutter size to set space between content and sidebar for blog archive', 'reina-core' ),
			...
			do_action( 'reina_core_action_after_blog_list_options_map', $list_tab );
			...
			$single_tab = $page->add_tab_element(
				array(
					...
					'description' => esc_html__( 'Settings related to blog single', 'reina-core' )
				)
			);

			// Hook to include additional options first in single blog options
			do_action( 'reina_core_action_first_blog_single_options_map', $single_tab );
			...
			$single_tab->add_field_element(
				array(
					...
					'description' => esc_html__( 'Use this option to enable/disable page title on blog single', 'reina-core' ),
			...
			$single_tab->add_field_element(
				array(
					...
					'title'         => esc_html__( 'Show Post Title in Title Area', 'reina-core' ),
					'description'   => esc_html__( 'Enabling this option will show post title in title area on single post pages', 'reina-core' ),
			...
			$single_tab->add_field_element(
				array(
					...
					'title'       => esc_html__( 'Sidebar Layout', 'reina-core' ),
					'description' => esc_html__( 'Choose default sidebar layout for blog single', 'reina-core' ),
					'options'     => reina_core_get_select_type_options_pool( 'sidebar_layouts' )
				)
			);
			...
			if ( ! empty( $custom_sidebars ) && count( $custom_sidebars ) > 1 ) {
				$single_tab->add_field_element(
					array(
						...
						'title'         => esc_html__( 'Custom Sidebar', 'reina-core' ),
						'description'   => esc_html__( 'Choose a custom sidebar to display on blog single', 'reina-core' ),
						'options'       => $custom_sidebars
					)
				);
			}
			...
			// Hook to include additional options after module options
			do_action( 'reina_core_action_after_blog_single_options_map', $single_tab );
			
			// Hook to include additional options after module options
			do_action( 'reina_core_action_after_blog_options_map', $page );
		}
	}
	
	add_action( 'reina_core_action_default_options_init', 'reina_core_add_blog_options', reina_core_get_admin_options_map_position( 'blog' ) );
}
