...
if ( ! function_exists( 'reina_core_add_simple_line_icons_to_collection' ) ) {
	/**
	 ...
	 * @return array
	 */
	function reina_core_add_simple_line_icons_to_collection( $icons ) {
		$icons[] = 'ReinaCoreSimpleLineIconsPack';
		
		return $icons;
	}
	
	add_filter( 'qode_framework_filter_add_icon', 'reina_core_add_simple_line_icons_to_collection' );
}

if ( class_exists( 'QodeFrameworkIconPack' ) ) {
	class ReinaCoreSimpleLineIconsPack extends QodeFrameworkIconPack {
		...
		public function get_style_url() {
			return REINA_CORE_INC_URL_PATH . '/icons/' . $this->get_base() . '/assets/css/' . $this->get_base() . '.min.css';
		}
