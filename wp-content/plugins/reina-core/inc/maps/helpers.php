...
if ( ! function_exists( 'reina_core_get_google_maps_api_key' ) ) {
	/**
	 ...
	 * @return bool|string
	 */
	function reina_core_get_google_maps_api_key( $return_type = 'option' ) {
		$option = reina_core_get_option_value( 'admin', 'qodef_maps_api_key' );
...
if ( ! function_exists( 'reina_core_set_multiple_map_variables' ) ) {
	/**
	 ...
	 */
	function reina_core_set_multiple_map_variables( $query = array() ) {
		$map_variables = array();
		...
		if ( is_array( $query ) && count( $query ) ) {
			...
			if (!empty($items)) {
				foreach ($items as $id => $title) {
					$map_variables['addresses'][] = reina_core_generate_map_params( $id, $query['post_type'] );
				}
			}
		}
...
if ( ! function_exists( 'reina_core_get_multiple_map' ) ) {
	/**
	 ...
	 * @return string
	 */
	function reina_core_get_multiple_map( $query = array() ) {
		$addresses = reina_core_set_multiple_map_variables( $query );
		...
		if ( reina_core_get_google_maps_api_key( 'is_enabled' ) ) {
			...
			$html = '<p id="qodef-multiple-map-notice">' . esc_html__( 'In order for the map functionality to be enabled please input the Google Map API key in the General section of the Reina Options', 'reina-core' ) . '</p>';
		}

		do_action( 'reina_core_action_after_multiple_map' );
...
if ( ! function_exists( 'reina_core_marker_info_template' ) ) {
	/**
	 * Template with placeholders for marker info window
	 *
	 * uses underscore templates
	 */
	function reina_core_marker_info_template() {
	...
	add_action( 'reina_core_action_after_multiple_map', 'reina_core_marker_info_template' );
}

if ( ! function_exists( 'reina_core_marker_template' ) ) {
	/**
	 * Template with placeholders for marker
	 */
	function reina_core_marker_template() {
	...
	add_action( 'reina_core_action_after_multiple_map', 'reina_core_marker_template' );
}
...
if ( ! function_exists( 'reina_core_generate_map_params' ) ) {
	/**
	 ...
	 * @return array
	 */
	function reina_core_generate_map_params( $item_id, $post_type ) {
		$map_params = array();
		...
		//get address params
		$address_array = apply_filters( 'reina_core_filter_address_params', array(), $item_id, $post_type );
