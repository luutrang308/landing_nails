...
if ( ! function_exists( 'reina_core_fullscreen_menu_typography_options' ) ) {
	/**
	 * Function that add additional header menu layout global options
	 *
	 * @param object $page
	 */
	function reina_core_fullscreen_menu_typography_options( $page ) {
		...
		if ( $page ) {
			...
			$typography_section = $page->add_section_element(
				array(
					...
					'title' => esc_html__( 'Fullscreen Menu Typography', 'reina-core' )
				)
			);
	...
	add_action( 'reina_core_action_after_fullscreen_menu_options_map', 'reina_core_fullscreen_menu_typography_options', 15 );
}