...
class ReinaCoreTitles {
	...
	public function __construct() {
		...
		add_filter( 'reina_filter_page_title_classes', array( $this, 'add_page_title_classes' ) );
		
		// Add title area inline styles
		add_filter( 'reina_filter_add_inline_style', array( $this, 'add_inline_styles' ) );
	}
	...
	function include_layouts() {
		...
		foreach ( glob( REINA_CORE_INC_PATH . '/title/dashboard/*/*.php' ) as $admin ) {
			include_once $admin;
		}
		
		foreach ( glob( REINA_CORE_INC_PATH . '/title/layouts/*/include.php' ) as $layout ) {
			include_once $layout;
		}
	}
	
	function set_variables() {
		$layout_meta = reina_core_get_post_value_through_levels( 'qodef_title_layout' );
		$layouts     = apply_filters( 'reina_core_filter_register_title_layouts', $layouts = array() );
	...
	function render_template() {
		...
		if ( ! empty( $title_object ) ) {
			$template_hook = $title_object->overriding_whole_title ? 'reina_filter_title_template' : 'reina_filter_title_content_template';
	...
	function add_page_title_classes( $classes ) {
		$layout             = reina_core_get_post_value_through_levels( 'qodef_title_layout' );
		$text_alignment     = reina_core_get_post_value_through_levels( 'qodef_page_title_text_alignment' );
		$vertical_alignment = reina_core_get_post_value_through_levels( 'qodef_page_title_vertical_text_alignment' );
	...
	function add_inline_styles( $style ) {
		...
		$height           	  	  = reina_core_get_post_value_through_levels( 'qodef_page_title_height' );
		$height_mobile     	  	  = reina_core_get_post_value_through_levels( 'qodef_page_title_height_on_smaller_screens' );
		$title_padding    	  	  = apply_filters( 'reina_core_filter_title_padding', 0 );
		$title_padding_mobile 	  = apply_filters( 'reina_core_filter_title_padding_mobile', 0 );
		$title_vertical_alignment = reina_core_get_post_value_through_levels( 'qodef_page_title_vertical_text_alignment' );
		$background_color 	  	  = reina_core_get_post_value_through_levels( 'qodef_page_title_background_color' );
		$image_params     	  	  = reina_core_get_page_title_image_params();
		...
		$background_svg_stroke = reina_core_get_post_value_through_levels( 'qodef_page_title_background_svg_stroke' );
		$background_svg_fill   = reina_core_get_post_value_through_levels( 'qodef_page_title_background_svg_fill' );
		
		if ( ! empty( $background_svg_stroke ) ) {
			$svg_styles['stroke'] = $background_svg_stroke;
		}

		if ( ! empty( $background_svg_fill ) ) {
			$svg_styles['fill'] = $background_svg_fill;
		}

		if ( ! empty( $svg_styles ) ) {
			$style .= qode_framework_dynamic_style( '.qodef-page-title .qodef-m-svg', $svg_styles );
		}

		$title_styles = array();

		$title_color = reina_core_get_post_value_through_levels( 'qodef_page_title_color' );
