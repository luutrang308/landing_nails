...
if ( ! function_exists( 'reina_core_add_custom_font_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function reina_core_add_custom_font_shortcode( $shortcodes ) {
		$shortcodes[] = 'ReinaCoreCustomFontShortcode';
		
		return $shortcodes;
	}
	
	add_filter( 'reina_core_filter_register_shortcodes', 'reina_core_add_custom_font_shortcode' );
}

if ( class_exists( 'ReinaCoreShortcode' ) ) {
	class ReinaCoreCustomFontShortcode extends ReinaCoreShortcode {
		
		public function __construct() {
			$this->set_layouts( apply_filters( 'reina_core_filter_custom_font_layouts', array() ) );
			
			parent::__construct();
		}
		
		public function map_shortcode() {
			$this->set_shortcode_path( REINA_CORE_SHORTCODES_URL_PATH . '/custom-font' );
			$this->set_base( 'reina_core_custom_font' );
			$this->set_name( esc_html__( 'Custom Font', 'reina-core' ) );
			$this->set_description( esc_html__( 'Shortcode that displays custom font with provided parameters', 'reina-core' ) );
			...
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'title',
				'title'      => esc_html__( 'Title Text', 'reina-core' )
			) );
			$this->set_option( array(
				'field_type'    => 'select',
				'name'          => 'title_tag',
		...
		public function render( $options, $content = null ) {
			...
			return reina_core_get_template_part( 'shortcodes/custom-font', 'variations/'.$atts['layout'].'/templates/custom-font', '', $atts );
		}
