...
if ( ! function_exists( 'reina_core_add_image_gallery_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function reina_core_add_image_gallery_shortcode( $shortcodes ) {
		$shortcodes[] = 'ReinaCoreImageGalleryShortcode';
		
		return $shortcodes;
	}
	
	add_filter( 'reina_core_filter_register_shortcodes', 'reina_core_add_image_gallery_shortcode' );
}

if ( class_exists( 'ReinaCoreListShortcode' ) ) {
	class ReinaCoreImageGalleryShortcode extends ReinaCoreListShortcode {
		
		public function map_shortcode() {
			$this->set_shortcode_path( REINA_CORE_SHORTCODES_URL_PATH . '/image-gallery' );
			$this->set_base( 'reina_core_image_gallery' );
			$this->set_name( esc_html__( 'Image Gallery', 'reina-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds image gallery element', 'reina-core' ) );
			...
			$this->set_option( array(
				...
				'name'       => 'images',
				'multiple'   => 'yes',
				'title'      => esc_html__( 'Images', 'reina-core' ),
			) );
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'image_size',
			...
			$this->map_list_options( array(
				...
				'group'            => esc_html__( 'Gallery Settings', 'reina-core' )
			) );
		}

		public static function call_shortcode( $params ) {
			$html = qode_framework_call_shortcode( 'reina_core_image_gallery', $params );
			$html = str_replace( "\n", '', $html );
		...
		public function render( $options, $content = null ) {
			...
			return reina_core_get_template_part( 'shortcodes/image-gallery', 'templates/image-gallery', $atts['behavior'], $atts );
		}
